...
### Task 6.2: 테스트 완료 (Task 4.3 잔여 항목 마무리) — ⚠️ 부분 완료

- [x] `lib/notion.ts` 단위 테스트 — `getInvoiceList` 정상/오류/빈결과/속성누락 5개 시나리오 (Vitest)
- [ ] `lib/pdf.tsx` 통합 테스트 — `InvoicePDF` 렌더링 및 버퍼 생성 검증
- [x] `app/api/generate-pdf/route.ts` API Route 테스트 — 정상/400/404/500 시나리오 (Vitest, mock 처리)
- [x] Playwright E2E 테스트 — 관리자 인증 플로우 6개 시나리오 (로그인/로그아웃/리다이렉트)
- [x] Playwright E2E 테스트 — 공개 페이지 기본 시나리오 (랜딩/관리자링크/404)
...
**작성일**: 2026-06-28
**최종 수정**: 2026-06-28 (Phase 6 Task 6.2 테스트 체크박스 실제 구현 반영)
